...
## Вспомогательные функции `Copy.___`

### `Copy.shallow(orig)`
Не рекурсивно копирует содержимое таблицы orig в новую таблицу.

Предположительно, устанавливает на новую таблицу метатаблицу orig.

### `Copy.deep(orig)`
Рекурсивно копирует содержимое таблицы orig в новую таблицу.
Обратите внимание, что `pairs(table)` не итерируется по
`getmetatable(table).__index`, поэтому при `__pairs` по умолчанию
все будет не так уж и плохо.

Предположительно, также рекурсивно копирует и устанавливает нужные метатаблицы.

### `Copy.localizeField(table, field)`
Представим ситуацию:
```lua
A = {
    new = Construct,
...
a = A:new()
```
Теперь, если мы в `a` попробуем изменить поле field,
мы можем случайно изменить исходное поле A.field:
```lua
a.field.x = 1
print(A.field.x) --> 1
```
При этом вот это не поможет, ведь `field` останется той же самой
таблицей, хоть ссылка на нее и появится в `a`
(а до этого доступ к `field` был через `getmeatatable(a).__index`):
```lua
a.field = a.field
a.field.x = 1
print(A.field.x) --> 1
```
Для решения данной проблемы сделан простой метод `Copy.localizeField`:
```lua
Copy.localizeField(a, "field")
a.field.x = 1
print(A.field.x) --> nil
```
**Внимание!** Будте осторожны, ведь `Copy.localizeField` делает `Copy.deep`.
Обращайте внимание на влияние данных манипуляций на производительность.
Иногда может быть лучше просто создать новый объект.
